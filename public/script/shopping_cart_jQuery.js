...
// products.forEach((produk) => {
  ...
  $.each(products, function( _, produk) { 
  // const card = document.createElement("div");
  const card = $("<div></div>")
  .addClass("grid grid-cols-2 mb-4 items-center p-4 border rounded-lg shadow-lg")
  .html(`
...
function updateItem(){
  ...
  $("#detail-minuman").html(comp); 

  const totalHarga = keranjangProduct.reduce((sum, produk) => sum + (produk.price * produk.quantity), 0);
  // const total = document.getElementById("total");
  // $("#total").html(`TOTAL : ${formatRupiah(totalHarga)}`);
  // const checkout = document.getElementById("checkout");
  
  $("#checkout").html(`<button class="bg-orange-600 active:bg-orange-400 transition w-36 h-10 rounded-lg text-white font-bold" onclick="checkout()">Checkout</button>`);
...
function tambahKeranjang(id){
  ...
  //mencari index keranjang
  const keranjangData = keranjangProduct[keranjangProduct.findIndex(pr => pr.id === id)];
  if (!keranjangData) {
    // menecek apakah data ada dalam keranjang jika tidak ada maka tambahkan
